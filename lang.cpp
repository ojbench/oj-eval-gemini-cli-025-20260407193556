...
ValuePtr CallExpression::eval(Context &ctx) const {
  ...
  if (func == "+") {
    ...
    if (x != 0 && y != 0) { global_trace.push_back(5); global_trace.push_back(std::min(x, y)); global_trace.push_back(std::max(x, y)); }
    return std::make_shared<IntValue>(x + y);
  } else if (func == "-") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    if (y != 0) { global_trace.push_back(6); global_trace.push_back(x); global_trace.push_back(y); }
    return std::make_shared<IntValue>(x - y);
  } else if (func == "*") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    if (x != 0 && x != 1 && y != 0 && y != 1) { global_trace.push_back(7); global_trace.push_back(std::min(x, y)); global_trace.push_back(std::max(x, y)); }
    return std::make_shared<IntValue>(x * y);
  } else if (func == "/") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    ...
    if (y != 1) { global_trace.push_back(8); global_trace.push_back(x); global_trace.push_back(y); }
    return std::make_shared<IntValue>(x / y);
  } else if (func == "%") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    ...
    global_trace.push_back(9); global_trace.push_back(x); global_trace.push_back(y);
    return std::make_shared<IntValue>(x % y);
  } else if (func == "<") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    global_trace.push_back(10); global_trace.push_back(x); global_trace.push_back(y);
    return std::make_shared<IntValue>(x < y);
  } else if (func == ">") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    global_trace.push_back(11); global_trace.push_back(x); global_trace.push_back(y);
    return std::make_shared<IntValue>(x > y);
  } else if (func == "<=") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    global_trace.push_back(12); global_trace.push_back(x); global_trace.push_back(y);
    return std::make_shared<IntValue>(x <= y);
  } else if (func == ">=") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    global_trace.push_back(13); global_trace.push_back(x); global_trace.push_back(y);
    return std::make_shared<IntValue>(x >= y);
  } else if (func == "==") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    global_trace.push_back(14); global_trace.push_back(std::min(x, y)); global_trace.push_back(std::max(x, y));
    return std::make_shared<IntValue>(x == y);
  } else if (func == "!=") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    global_trace.push_back(15); global_trace.push_back(std::min(x, y)); global_trace.push_back(std::max(x, y));
    return std::make_shared<IntValue>(x != y);
  } else if (func == "||") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    global_trace.push_back(16); global_trace.push_back(std::min(x, y)); global_trace.push_back(std::max(x, y));
    return std::make_shared<IntValue>(x || y);
  } else if (func == "&&") {
    requireArity(2);
    int x = readInt(0), y = readInt(1);
    global_trace.push_back(17); global_trace.push_back(std::min(x, y)); global_trace.push_back(std::max(x, y));
    return std::make_shared<IntValue>(x && y);
  } else if (func == "!") {
    requireArity(1);
    int x = readInt(0);
    global_trace.push_back(18); global_trace.push_back(x);
    ...
    global_trace.push_back(1); global_trace.push_back(index); global_trace.push_back(array->contents[index]);
    ...
    global_trace.push_back(2); global_trace.push_back(index); global_trace.push_back(value);
